...
function makeScene(kind, accent) {
  ...
  if (kind === "globe" || kind === "storm" || kind === "pins") {
    ...
    // FinOps desk: stacked charge slabs + orbiting receipt pip.
    for (let i = 0; i < 4; i++) {
      const slab = new THREE.Mesh(
        new THREE.BoxGeometry(1.05 - i * 0.08, 0.14, 0.55),
        mat.clone()
      );
      slab.material.transparent = true;
      slab.material.opacity = 0.38 + i * 0.12;
      slab.position.y = -0.42 + i * 0.22;
      slab.rotation.y = (i - 1.5) * 0.08;
      group.add(slab);
    }
    group.add(new THREE.Mesh(new THREE.TorusGeometry(0.82, 0.025, 8, 48), wire));
    const pip = new THREE.Mesh(
      new THREE.SphereGeometry(0.08, 12, 12),
      new THREE.MeshBasicMaterial({ color: 0xffffff })
    );
    pip.position.set(0.55, 0.35, 0.25);
    pip.name = "needle";
    group.add(pip);
  } else if (kind === "compare") {
    // Bake-off: two candidate cores under one acceptance ring.
    const left = new THREE.Mesh(new THREE.IcosahedronGeometry(0.34, 0), wire);
    left.position.x = -0.42;
    group.add(left);
    const right = new THREE.Mesh(new THREE.OctahedronGeometry(0.32), mat.clone());
    right.material.transparent = true;
    right.material.opacity = 0.7;
    right.position.x = 0.42;
    group.add(right);
    group.add(new THREE.Mesh(new THREE.TorusGeometry(0.88, 0.03, 8, 56), wire));
