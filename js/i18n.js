...
const LOCALE_VERSION = "20260818b"; // catalog release 20260818b
...
function localizeAtlasLink(anchor, lang = current) {
  if (!anchor || !anchor.href) return;
  try {
    const url = new URL(anchor.href, location.href);
    if (url.hostname !== "atlas.modelmarket.dev") return;
...
function syncAtlasLinks(lang = current) {
  document.querySelectorAll('a[href*="atlas.modelmarket.dev"]').forEach((anchor) => {
    localizeAtlasLink(anchor, lang);
  });
}
...
async function loadDict(lang) {
  ...
  const res = await fetch(`${basePath()}locales/${lang}.json?v=${LOCALE_VERSION}`);
...
document.addEventListener("click", (event) => {
  localizeAtlasLink(event.target.closest && event.target.closest("a"), current);
}, true);
