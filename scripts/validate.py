...
def main() -> None:
    ...
    audited_demos = {
        ...
        "england-river-gauge": ("gaia.river.read@v1", "0.001"),
        "rhine-nl-gauge": ("gaia.river.read@v1", "0.001"),
        "usace-reservoir": ("gaia.reservoir.read@v1", "0.001"),
        "ireland-synoptic": ("gaia.weather.read@v1", "0.001"),
    }
    for idea_id, idea in idea_registry.items():
        ...
        ref = idea.get("referenceDesk") if isinstance(idea, dict) else None
        if ref is not None:
            if not isinstance(ref, dict):
                fail(f"{idea_id}: referenceDesk must be an object")
            href = str(ref.get("href") or "")
            card = str(ref.get("card") or "")
            desk_id = str(ref.get("id") or "")
            if not href.startswith("https://"):
                fail(f"{idea_id}: referenceDesk.href must be https")
            if card not in idea_registry:
                fail(f"{idea_id}: referenceDesk.card {card!r} is missing from ideas")
            if desk_id not in ("emberline", "tideline", "solrecord", "seamark", "plinth"):
                fail(f"{idea_id}: unknown referenceDesk.id {desk_id!r}")
