 ...
 */
import { mountPreview } from "./preview3d.js?v=20260915b";
import { currentLang, onDict, getDict } from "./i18n.js?v=20260915b";
...
const els = {
  ...
  refWrap: document.getElementById("idea-ref"),
  refChip: document.getElementById("idea-ref-chip"),
  refTitle: document.getElementById("idea-ref-title"),
  refTeaser: document.getElementById("idea-ref-teaser"),
  refNote: document.getElementById("idea-ref-note"),
  refOpen: document.getElementById("idea-ref-open"),
  refCard: document.getElementById("idea-ref-card"),
...
function paintReferenceDesk(dict) {
  const ref = cachedIdea?.referenceDesk;
  if (!els.refWrap) return;
  if (!ref || ref.card === id || cachedIdea?.chip === "example") {
    els.refWrap.hidden = true;
    return;
  }
  const cardPrefix = `idea.${ref.card}`;
  els.refWrap.hidden = false;
  if (els.refChip) els.refChip.textContent = dict["chip.example"] || "example desk";
  if (els.refTitle) els.refTitle.textContent = dict[`${cardPrefix}.title`] || dict[`desk.${ref.id}.name`] || ref.id;
  if (els.refTeaser) els.refTeaser.textContent = dict[`${cardPrefix}.teaser`] || "";
  if (els.refNote) els.refNote.textContent = dict["idea.ref.note"] || "";
  if (els.refOpen) {
    els.refOpen.href = ref.href || "#";
    els.refOpen.textContent = dict["idea.ref.open"] || "Open the live desk →";
    els.refOpen.onclick = () => emitInteraction("reference_desk", ref.id);
  }
  if (els.refCard) {
    els.refCard.href = `${base}ideas.html?v=${DATA_VERSION}&id=${encodeURIComponent(ref.card)}`;
    els.refCard.textContent = dict["idea.ref.card"] || "Read the desk card →";
  }
}
