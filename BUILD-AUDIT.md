...
| **Attestation ≠ physical-world truth** | Ed25519 proves *who signed* and whether the envelope changed, not that the sensor isn’t in a fridge or an upstream registry is independently correct. Need history, multiple sources, domain checks, and partner SLA. |
...
## 09 — Flood hydrology evidence

**Portal claim:** US NWS flood alerts + river gauges in a bbox → cited hydrology pack + receipt.

### Pitfalls
- NWS CAP is **free**. A wrap with no pairing, no receipt, and no agent billing is AutoWarn without the brand.
- Floodbase / Fathom sell **indices and quotes** (50-year history, Liberty Quote API, Mar 2026). We have alerts + gauges. Do not impersonate them.
- WaterWatch JSON is retired; do not scrape GloFAS. Coverage is US-centric — EU/global bbox will often refuse honestly.
- Pairing is **co-presence**, not a hydrologic model. Saying “river confirms the flood” is a causation claim we do not make.

### Engineering hard parts
- Default brief already includes flood/river; the product layer is named basins, watchbox poll, ticket attach.
- Dual-source refuse: empty flood **and** empty river still fails closed if those are the requested layers.
- Attribution: NWS CAP + USGS-class gauges; keep licenses on the pack.

### Improved problem statement
> **Vertical of** `atlas.situation.brief@v1` / `atlas.watchbox.check@v1`: for a US bbox, return LIVE flood alerts and river gauges with the hydrology pairing driver when both are present, plus a content receipt. Not a flood model or parametric quote. Success = one terminal/yard or cat-ops desk attaching packs to weekly exception review for 4 weeks.

---

## 10 — Campus physical nowcast

**Portal claim:** Named campus bbox → default-layer situation brief for NOC / facilities.

### Pitfalls
- First Street (Jun 2026) and Schneider ClimVaR are **siting / 30-year** products. A nowcast brief does not replace them; claiming it does is a lie.
- Generic “situation brief” with extra adjectives is not a use case. The buyer is a **named campus**.
- Sparse LIVE in a suburban campus bbox is likely. Refuse must stay loud.
- Safety/insurance language (“certify the site”) is out of scope.

### Engineering hard parts
- Site registry (name → bbox), poll cadence, ticket/PDF attach.
- Layer policy: default map layers, not energy/iot.
- Multi-campus tenants and ACLs if this becomes SaaS.

### Improved problem statement
> **Vertical of** `atlas.situation.brief@v1`: for a named campus bbox, emit a fileable LIVE nowcast (drivers + citations + receipt). Application layer = site list + poll + ticket attach. Success = one colo/hyperscaler facilities team using briefs in daily ops for 4 weeks; climate siting explicitly out of scope.
...
| Order | Idea | Why |
|---|---|---|
| 1 | **Support/Sales Answer Evidence Pack** | Fastest demonstrable desk product on Metis + receipt rails; buyer job is obvious |
| 2 | **Agent Spend Policy & Chargeback Desk** | Strongest monetization of ARGUS · Hub · HEPHAESTUS · LOGOS; refuse invented spend |
| 3 | **Capability Acceptance Bake-off** | Signature alexar76 loop: Market trial + THEMIS + Metis/AWR — buyer rubric only |
| 4 | **Capability SLA Incident Pack** | Same economy rails; package a dispute window without assigning liability |
| 5 | **Fire + Weather Evidence Snapshot** | Callable ATLAS SKU; FIRMS and/or EFFIS; bounded weather; no unvalidated risk score |
| 6 | **Verified Watchbox** | Callable SKU with bbox, layer filtering, LIVE/SIM flags, matches, and content receipt |
| 7 | **Flood Hydrology Evidence** | Paying US flood-ops adjacency; uses shipped flood+river pairing; stay evidence not Floodbase |
| 8 | **Campus Physical Nowcast** | Paying DC/colo ops adjacency; vertical of situation.brief; siting software stays out of scope |
| Later | Contract-Drift Desk (design partner), US Water-Intake, Finland Winter Lane, situation brief, publisher kit, parametric | Heavier partner / geography / BD |

### Critical boundaries (economy desk set)

- Evidence Pack proves origin and integrity of an answer, not absolute truth.
- Spend Desk must mark missing receipts **unreconciled** — never invent spend.
- Bake-off scores only the buyer’s task set and rubric — not a universal trust score.
- SLA Pack does not assign legal liability or auto-refund.
- Contract Drift is not a source-code audit or a guarantee of future behaviour.
- Finland Winter Lane is not EU-wide road weather.
- Water Intake is not a lab analysis or certification of compliance.
- AWR is a **draft** portable envelope: every product ships with its own verifier and must not depend on external AWR recognition.
